...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, attn_mask=None, encoder_hidden_states=None) -> torch.Tensor:
        B, N, C = x.shape
        ...
        if encoder_hidden_states is not None:
            ...
            kv = self.kv(encoder_hidden_states).reshape(B, encoder_hidden_states.shape[1], 2, self.num_heads, self.head_dim).permute(2, 0, 3, 1, 4)
            k, v = kv.unbind(0)  # Each of shape B, num_heads, encoder_seq_len, head_dim
            ...
            if self.fused_attn:
                ...
                q = q * self.scale
                attn = q @ k.transpose(-2, -1)  # B, num_heads, N, encoder_seq_len
        ...
        # Regular self-attention case
        else:
            qkv = torch.cat([self.q(x), self.kv(x)], dim=-1).reshape(B, N, 3, self.num_heads, self.head_dim).permute(2, 0, 3, 1, 4)
            q, k, v = qkv.unbind(0)
            q, k = self.q_norm(q), self.k_norm(k)
...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, c, attn_mask=None, encoder_hidden_states=None):
        shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = self.adaLN_modulation(c).chunk(6, dim=1)
        x = x + gate_msa.unsqueeze(1) * self.attn(modulate(self.norm1(x), shift_msa, scale_msa), attn_mask=attn_mask, encoder_hidden_states=encoder_hidden_states)
...
class DiT(PreTrainedModel):
    ...
    def __init__(
        ...
        self.reasoning_as_cond = config.reasoning_layer_id > -1
        if self.reasoning_as_cond:
            self.cond_reasoning_emb = nn.Linear(config.cond_dim+config.state_dim, config.n_emb)
        ...
        self.hidden_states_for_cosattn = nn.Linear(config.cond_dim, config.n_emb)
        ...
        self.reasoning_layer_id = config.reasoning_layer_id
    ...
    def forward(self, actions, hidden_states, states, is_pad, reasoning=None, encoder_hidden_states=None, attention_mask=None):
        """
        ...
        if actions is not None:  # training time
            ...
            reasoning = reasoning.repeat(num_noise_samples, 1, 1)
            ...
            encoder_hidden_states = encoder_hidden_states.repeat(num_noise_samples, 1, 1) if encoder_hidden_states is not None else None
            ...
            noise_pred = self.model_forward(
                ...
                states=states,
                reasoning=reasoning,
                reasoning_layer_id=self.reasoning_layer_id,
    ...
    def model_forward(self, x, t, global_cond, states, reasoning=None, reasoning_layer_id=-1, encoder_hidden_states=None, attention_mask=None):
        """
        Forward pass of DiT.
        x: (N, T, input_dim)
        ...
        if encoder_hidden_states is not None:
            # encoder_hidden_states = self.global_1d_pool(encoder_hidden_states.permute(0, 2, 1)).squeeze(-1)
            # encoder_hidden_states = self.norm_after_pool(encoder_hidden_states)
            encoder_hidden_states = self.hidden_states_for_cosattn(encoder_hidden_states)

        # c = t + global_cond.sum(dim=1)  # (N, D)
        c = t + global_cond  # (N, D)

        if self.reasoning_as_cond:
            reasoning = self.global_1d_pool(reasoning.permute(0, 2, 1)).squeeze(-1)
            reasoning = self.norm_after_pool(reasoning)
            reasoning = torch.cat([reasoning, states], dim=-1) if states is not None else reasoning
            global_reasoning_cond = self.cond_reasoning_emb(reasoning)
            c_reasoning = t + global_reasoning_cond

        for i,block in enumerate(self.blocks):
            add_encoder_hidden_states = [encoder_hidden_states, None]
            if (not self.reasoning_as_cond) or (i < self.reasoning_layer_id):
                x = block(x, c, attn_mask=attention_mask, encoder_hidden_states=add_encoder_hidden_states[i%2])  # (N, T, D)
            else:
                x = block(x, c_reasoning, attn_mask=None, encoder_hidden_states=add_encoder_hidden_states[i%2])

        if not self.reasoning_as_cond:
            x = self.final_layer(x, c)  # (N, T, output_dim)    
        else:
            x = self.final_layer(x, c_reasoning) # (N, T, output_dim)
        return x
...
DiT_models = {
    'DiT-XL': DiT_XL, 'DiT-L': DiT_L, 'DiT-B': DiT_B, 'DiT-S': DiT_S, 'DiT_H':DiT_H
}
