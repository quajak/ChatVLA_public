...
class QWen2VLATrainer(Trainer):
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        with self.compute_loss_context_manager():
            ###############################modified##################################
            loss, outputs = self.compute_loss(model, inputs, return_outputs=True) # change return_outputs to True
        ...
        if outputs is not None:
            return loss['loss'] / self.args.gradient_accumulation_steps, loss, outputs[1]
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                with self.accelerator.accumulate(model):
                    tr_loss_step, all_loss, expert_traffic = self.training_step(model, inputs) # modified,return all_loss
                ####################################################
                ...
                if (
                    ...
                    self._maybe_log_save_evaluate(tr_loss, model, trial, epoch, ignore_keys_for_eval, all_loss=custom_loss, expert_traffic=expert_traffic)
                    ###############################################################################################################################################
                else:
    ...
    def _maybe_log_save_evaluate(self, tr_loss, model, trial, epoch, ignore_keys_for_eval, all_loss=None, expert_traffic=None):
        if self.control.should_log and self.state.global_step > self._globalstep_last_logged:
            ...
            expert_traffic = self._nested_gather(expert_traffic)
            row_sum = expert_traffic.sum(dim=1, keepdim=True)
            expert_traffic_prob = expert_traffic / row_sum
            ################################################################################################################
            ...
            logs["expert_traffic"] = expert_traffic_prob
            expert_traffic_prob -= expert_traffic_prob
            ##############################################modified########################################################
    ...
    #
    #     Modified to handle tensor logging directly.
    #     """
    #     # 如果 logs 中包含一个张量，记录它
    #     if "expert_traffic" in logs:
    #         tensor = logs["expert_traffic"]
    #         logger.experiment.add_histogram(
    #             "Tensor Histogram", tensor, self.state.global_step
    #         )
    #         logs = logs.pop("expert_traffic")
    #
